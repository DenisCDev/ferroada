...
Vercel Deployment Protection: IP da VPS **ou** `x-vercel-protection-bypass`. Este pack injeta `ORIGIN_SECRET_HEADER` (já vem `x-vercel-protection-bypass`) com `ORIGIN_SECRET`. Cole o mesmo token no painel da Vercel. O processo não loga o valor.
