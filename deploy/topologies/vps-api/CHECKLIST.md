...
- Preencha `ALLOWED_HOSTS` com o host da API.
- CORS é do backend. O Ferroada não inventa `Access-Control-Allow-Origin`.
- Rotas fail-closed: `/api/`, `/auth/`, `/graphql` (inspeção completa; body truncado ou encoding não suportado não seguem).
