...
1. **Recomendada — não proxyar a Cloud.** Este `ferroada.toml` só tem o app. O browser aponta `SUPABASE_URL` para `https://PROJECT_REF.supabase.co`.
2. **BFF por host dedicado, não por path.** Copie o bloco de `ferroada.toml.bff.example` para o `ferroada.toml`. Preencha `{{SUPABASE_HOST}}` (ex.: `supabase.exemplo.com`) e `PROJECT_REF`. A app aponta `SUPABASE_URL` para `https://{{SUPABASE_HOST}}`. Ajuste cookie, CORS e redirect URLs no GoTrue. O Host enviado ao upstream é o da Cloud.

O 0.6.0 só roteia por Host, nunca por prefixo de URL.
Não use `backend = "https://*.supabase.co"`.
