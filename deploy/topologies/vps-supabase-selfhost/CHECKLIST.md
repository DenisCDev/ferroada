# Checklist — `vps-supabase-selfhost`
...
1. Suba o stack oficial do Supabase **antes**. Este Compose espera a rede `${ORIGIN_NETWORK:-origin}` já criada (Kong a escutar 8000 nela).
2. Studio (`:3000` típico do Studio) **não** entra no listener público. Se precisar, SSH tunnel. Studio não é API.
3. Realtime/WebSocket: o 0.6.0 não inspeciona frames. O tráfego passa como túnel HTTP upgrade. Não há knob `bypass-explicit` neste binário — o `VISIBILIDADE.md` diz isso.
4. GoTrue JWT: Level 1 não valida assinatura. RLS no Postgres continua a ser a autorização.
