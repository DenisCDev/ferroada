...
**Fase 6 — bot/abuse (primeira fatia).** RiskIdentity soma fingerprint TLS (hash estável do ClientHello: versão + ciphers + compressão; **não** JA4 — Pingora 0.8.1 não expõe), fingerprint HTTP (nomes dos headers, sem valores), ASN MMDB opt-in (sem ficheiro → `asn = None`, o processo arranca), sessão/API key/`jwt.sub`. Quotas stuffing / enumeração / signup / recover por identidade composta, opt-in no TOML (`abuse = { ... }`); site sem o bloco fica como hoje. Score com razões enumeráveis (`stuffing`, `ua_weak`, `ip_rotate`, …) em `SecurityEvent.detail`, sem payload de senha. Challenge `none` | `pow` | `external` depois do limiar; browser `Accept: text/html` pode levar 403+HTML; API com JWT ou `Content-Type: application/json` não. Sem ML, sem feed de reputação, sem WebAuthn/CAPTCHA vendor. UA de 18 strings continua sinal fraco.
